...
#for row_count in range(1, 4) :
    ...
    UL_FREQ = float(sheet.cell_value(row_count, 17))
    ...
    try:
        ...
        Range3_Margin = margin[2]
        Range4_Margin = margin[3]
        Range5_Margin = margin[4]
        Screenshot = ""

        if TEST_BAND == 2 or TEST_BAND == 4 or TEST_BAND == 66:
            range4_msmt = str(Meas[0][3]/1000000) +'/ ' + str(Meas[1][3]) + '/ ' + str(Meas[2][3])
            range4_msmt_freq = str(Meas[0][3]/1000000)
            range4_msmt_abs= str(Meas[1][3])
            range4_msmt_limt= str(Meas[2][3])
            range5_msmt = str(Meas[0][4]/1000000) + '/ ' + str(Meas[1][4]) + '/ ' + str(Meas[2][4])
            range5_msmt_freq = str(Meas[0][4] / 1000000)
            range5_msmt_abs = str(Meas[1][4])
            range5_msmt_limt = str(Meas[2][4])
            lte_tx_result = {'UPLINK_FREQUENCY':[UL_FREQ],'Channel BW(MHz)':[TEST_BW],'UL RB Allocation': [TEST_RB], 'UL RB Start': [Start_RB], 'TX Power' : [TX_P], 'Spurious Peak Freq Range4(MHz)' : [range4_msmt_freq], 'Spurious Abs Power Range4(-dBm)' : [range4_msmt_abs], 'Spurious Limt power Range4(-dB)' : [range3_msmt_limt], 'Spurious Margin Range4(-dB)' : [Range4_Margin], 'Spurious Peak Freq Range5(MHz)' : [range5_msmt_freq], 'Spurious Abs Power Range5(-dBm)' : [range5_msmt_abs], 'Spurious Limt power Range5(-dB)' : [range5_msmt_limt], 'Spurious Margin Range5(-dB)' : [Range5_Margin], 'Screenshot': [Screenshot]}
        else:
            range3_msmt = str(Meas[0][2]/1000000) + '/ ' + str(Meas[1][2]) + '/' + str(Meas[2][2])
            range3_msmt_freq = str(Meas[0][2] / 1000000)
            range3_msmt_abs = str(Meas[1][2])
            range3_msmt_limt = str(Meas[2][2])
            range4_msmt = str(Meas[0][3]/1000000) + '/ ' + str(Meas[1][3]) + '/ ' + str(Meas[2][3])
            range4_msmt_freq = str(Meas[0][3] / 1000000)
            range4_msmt_abs = str(Meas[1][3])
            range4_msmt_limt = str(Meas[2][3])
            ...
            lte_tx_result = {'UPLINK_FREQUENCY':[UL_FREQ], 'Channel BW(MHz)' : [TEST_BW],
                              'UL RB Allocation': [TEST_RB], 'UL RB Start': [Start_RB],
                             'TX Power(-dB)': [TX_P], 'Spurious Peak Freq Range3(MHz)' : [range3_msmt_freq], 'Spurious Abs Power Range3(-dBm)' : [range3_msmt_abs], 'Spurious Limt power Range3(-dB)' : [range3_msmt_limt], 'Spurious Margin Range3(-dB)' : [Range3_Margin], 'Spurious Peak Freq Range4(MHz)' : [range4_msmt_freq], 'Spurious Abs Power Range4(-dBm)' : [range4_msmt_abs], 'Spurious Limt power Range4(-dB)' : [range4_msmt_limt], 'Spurious Margin Range4' : [Range4_Margin],
